...
FActorDefinition UActorBlueprintFunctionLibrary::MakeImprovedGnssDefinition()
{
  FActorDefinition Definition;
  bool Success;
  MakeImprovedGnssDefinition(Success, Definition);
  check(Success);
  return Definition;
}

void UActorBlueprintFunctionLibrary::MakeImprovedGnssDefinition(
    bool &Success,
    FActorDefinition &Definition)
{
  FillIdAndTags(Definition, TEXT("sensor"), TEXT("other"), TEXT("improvedgnss"));
  AddVariationsForSensor(Definition);
...
void UActorBlueprintFunctionLibrary::SetImprovedGnss(
    const FActorDescription &Description,
    AImprovedGnssSensor *Gnss)
{
  CARLA_ABFL_CHECK_ACTOR(Gnss);
  if (Description.Variations.Contains("noise_seed"))
  {
